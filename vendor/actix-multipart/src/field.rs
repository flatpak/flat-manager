...
impl InnerField {
    ...
    pub(crate) fn read_stream(
        ...
        // check boundary
        // LOCAL PATCH (flat-manager): upstream uses `len > 4`, which fails to treat a
        // buffer of exactly `\r\n--` (an inter-part boundary delimiter split by a
        // network chunk right after `--`) as a possible boundary. It then consumes
        // `\r\n--` as field body, misaligns, and silently drops the next field. Using
        // `>= 4` makes that case fall into `len < b_size` below and return Pending to
        // wait for the rest of the delimiter. See vendor/actix-multipart/README.flatmanager.md
